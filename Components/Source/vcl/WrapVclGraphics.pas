...
begin
  ...
  APyDelphiWrapper.DefineVar('tfEditControl', 'tfEditControl');
  APyDelphiWrapper.DefineVar('tfEndEllipsis', 'tfEndEllipsis');
  APyDelphiWrapper.DefineVar('tfPathEllipsis', 'tfPathEllipsis');
  APyDelphiWrapper.DefineVar('tfExpandTabs', 'tfExpandTabs');
  APyDelphiWrapper.DefineVar('tfExternalLeading', 'tfExternalLeading');
  APyDelphiWrapper.DefineVar('tfLeft', 'tfLeft');
  APyDelphiWrapper.DefineVar('tfModifyString', 'tfModifyString');
  APyDelphiWrapper.DefineVar('tfNoClip', 'tfNoClip');
  APyDelphiWrapper.DefineVar('tfNoPrefix', 'tfNoPrefix');
  APyDelphiWrapper.DefineVar('tfRight', 'tfRight');
  APyDelphiWrapper.DefineVar('tfRtlReading', 'tfRtlReading');
  APyDelphiWrapper.DefineVar('tfSingleLine', 'tfSingleLine');
  APyDelphiWrapper.DefineVar('tfTop', 'tfTop');
  APyDelphiWrapper.DefineVar('tfVerticalCenter', 'tfVerticalCenter');
  APyDelphiWrapper.DefineVar('tfWordBreak', 'tfWordBreak');
  APyDelphiWrapper.DefineVar('tfHidePrefix', 'tfHidePrefix');
  APyDelphiWrapper.DefineVar('tfNoFullWidthCharBreak', 'tfNoFullWidthCharBreak');
  APyDelphiWrapper.DefineVar('tfPrefixOnly', 'tfPrefixOnly');
  APyDelphiWrapper.DefineVar('tfTabStop', 'tfTabStop');
  APyDelphiWrapper.DefineVar('tfWordEllipsis', 'tfWordEllipsis');
  APyDelphiWrapper.DefineVar('tfComposited', 'tfComposited');

  APyDelphiWrapper.DefineVar('bsSolid', 'bsSolid');
  APyDelphiWrapper.DefineVar('bsClear', 'bsClear');
  APyDelphiWrapper.DefineVar('bsHorizontal', 'bsHorizontal');
  APyDelphiWrapper.DefineVar('bsVertical', 'bsVertical');
  APyDelphiWrapper.DefineVar('bsFDiagonal', 'bsFDiagonal');
...
begin
  ...
  with GetPythonEngine do begin
    _resName := nil;
    if PyArg_ParseTuple( args, 'iO:LoadFromResourceName',@_instance, @_resName ) <> 0 then
    begin
      if _resName <> nil then
        DelphiObject.LoadFromResourceName(_instance, PyObjectAsString(_resName));
      Result := ReturnNone;
    end
    else
      Result := nil;
...
begin
  ...
  with GetPythonEngine do begin
    x := 0;
    y := 0;
    if PyArg_ParseTuple( args, 'iiO:TextRect',@x, @y, @_text ) <> 0 then
    begin
      if _text <> nil then
        DelphiObject.TextOut(x, y, PyObjectAsString(_text));
      Result := GetPythonEngine.ReturnNone;
    end
    else
      Result := nil;
...
begin
  ...
  with GetPythonEngine do begin
    ...
    _rectO := nil;
    _text := nil;
    if (PyArg_ParseTuple( args, 'OiiO:TextRect',@_rectO, @x, @y, @_text ) <> 0) and
       CheckRectAttribute(_rectO, 'Rect', _rect) then
    begin
      if _text <> nil then
        DelphiObject.TextRect(_rect, x, y, PyObjectAsString(_text));
      Result := GetPythonEngine.ReturnNone;
    end
    else
      Result := nil;
