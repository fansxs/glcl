...
begin
  ...
  with GetPythonEngine do
  begin
    if PyArg_ParseTuple(AArgs, 'O:IsValidStyle', @LFileName) <> 0 then
    begin
      LIsValid := TStyleManager.IsValidStyle(PyObjectAsString(LFileName), LStyleInfo);
...
begin
  with GetPythonEngine do
  begin
    if PyArg_ParseTuple(AArgs, 'O:LoadFromFile', @LFileName) <> 0 then
    begin
      TStyleManager.LoadFromFile(PyObjectAsString(LFileName));
...
var
  LName: PPyObject;
  LAuthor: PPyObject;
  LAuthorEMail: PPyObject;
  LAuthorURL: PPyObject;
  LVersion: PPyObject;
begin
  Create(APythonType);
  with APythonType.Engine do
    if PyArg_ParseTuple(args, 'OOOOO:Create', @LName, @LAuthor, @LAuthorEMail, @LAuthorURL, @LVersion) <> 0 then
    begin
      FValue.Name := PyObjectAsString(LName);
      FValue.Author := PyObjectAsString(LAuthor);
      FValue.AuthorEMail := PyObjectAsString(LAuthorEMail);
      FValue.AuthorURL := PyObjectAsString(LAuthorURL);
      FValue.Version := PyObjectAsString(LVersion);
    end
end;
